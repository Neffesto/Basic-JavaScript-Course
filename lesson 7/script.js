...
const map = {
     ...
     * @param {{x: int, y: int}} barrierPoint Точка преграды.
    ...
    render(snakePointsArray, foodPoint, barrierPoint) {
        ...
        // Получаем элемент ячейки с преградой по точке barrierPoint.
...
/**
 * Объект преграды.
 * @property {int} x Координата X преграды.
 * @property {int} y Координата Y преграды.
 */
const barrier = {
    x: null,
    y: null,
    ...
    /**
     * Отдает координаты преграды.
     * @returns {{x: int, y: int}} Координаты преграды.
     */
    getCoordinatesBarrier() {
        return {
            x: this.x,
            y: this.y,
        }
    ...
    /**
     * Устанавливает координаты для преграды.
     * @param {{x: int, y: int}} point Новая точка с координатами преграды.
     */
    setCoordinatesBarrier(point) {
        this.x = point.x;
        this.y = point.y;
    },

    /**
     * Определяет соответствует ли точка на которой находится преграда той точке что была передана.
     * @param {{x: int, y: int}} point Точка, для проверки соответствия точке преграды.
     * @returns {boolean} true, если точки совпали, иначе false.
     */
    isOnPoint(point) {
        return this.x === point.x && this.y === point.y;
 ...
 * @property {barrier} barrier Объект преграды.
...
const game = {
    ...
    reset() {
        ...
        // Ставим преграду на карту в случайную пустую ячейку.
        this.barrier.setCoordinatesBarrier(this.getRandomFreeCoordinates());
    ...
    tickHandler() {
        ...
        if (!this.canMakeStep()) {
            return this.finish();
            // если следующий шаг преграда
        } else if (this.barrier.isOnPoint(this.snake.getNextStepHeadPoint())) {
            return this.finish();
        }
        ...
        if (this.food.isOnPoint(this.snake.getNextStepHeadPoint())) {
            ...
            this.food.setCoordinates(this.getRandomFreeCoordinates());
            // Ставим преграду в свободную ячейку.
            this.barrier.setCoordinatesBarrier(this.getRandomFreeCoordinates());
     ...
     */
    render() {
        this.map.render(this.snake.getBody(), this.food.getCoordinates(), this.barrier.getCoordinatesBarrier());
